...
local function no_arrows(direction, hint)
	return function()
		vim.notify("Usa le vim motions, non le frecce → prova '" .. hint .. "' per andare " .. direction, vim.log.levels.INFO, { title = "No arrows club" })
	end
end
...
local opencode = function(action, opts)
	return function() require("opencode")[action](opts) end
end
...
local plugins = {
	...
	{ gh("nvim-mini/mini.pairs"), config = function() require("mini.pairs").setup() end },
	{ gh("nvim-mini/mini.indentscope"), config = function() require("mini.indentscope").setup({ symbol = "│", options = { try_as_border = true } }) end },
	{ gh("nvim-mini/mini.icons"), config = function() require("mini.icons").setup() end },
	{ gh("nvim-mini/mini.surround"), config = function() require("mini.surround").setup() end },

	-- Mini.pick with border config
	{ gh("nvim-mini/mini.pick"), config = function() require("mini.pick").setup({ window = { config = { border = "rounded" } } }) end },

	-- Mini.clue
	{ gh("nvim-mini/mini.clue"), config = function()
	...
	-- Colorscheme
	{ gh("folke/tokyonight.nvim"), config = function() vim.cmd.colorscheme("tokyonight") end },

	-- File explorers
	{ gh("nickjvandyke/opencode.nvim") },
	{ gh("refractalize/oil-git-status.nvim"), config = function() require("oil-git-status").setup() end },
	{ gh("stevearc/oil.nvim"), config = function()
		require("oil").setup({
	...
	{ gh("neovim/nvim-lspconfig"), config = function()
		local lsp = vim.lsp
		lsp.config("lua_ls", {})
		lsp.enable("lua_ls")
		lsp.config("basedpyright", { settings = { python = { analysis = { diagnosticMode = "openFilesOnly" } } } })
		lsp.enable("basedpyright")
		lsp.config("jsonls", { settings = { json = { validate = { enable = true }, schemas = require("schemastore").json.schemas() } } })
		lsp.enable("jsonls")
	...
	-- Completion
	{ gh("saghen/blink.cmp"), build = "cargo build --release", config = function()
	...
	-- Treesitter
	{ gh("nvim-treesitter/nvim-treesitter"), build = ":TSUpdate", config = function()
	...
	-- Formatter
	{ gh("stevearc/conform.nvim"), config = function()
